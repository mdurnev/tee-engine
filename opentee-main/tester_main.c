...
int main(int argc, char **argv)
{
	...
	char *test_ca = getenv("OPENTEE_TEST_CA");
	int is_launcher = 0;
	pid_t manager_pid = 0;
	...
	control_params.launcher_pid = getpid();
	manager_pid = fork();
	if (manager_pid == -1) {
		/* failed to fork */
		return -1;
	} else if (manager_pid != 0) {
		/* parent process will become the launcher */
		close(sockfd[0]);
		control_params.comm_sock_fd = sockfd[1];
		lib_to_load = "libLauncher1Api.so"; /* this code does not fork (it cannot launch more than one TA at a time) */
		strncpy(proc_name, "tee_launcher", MAX_PR_NAME);
		prctl(PR_SET_PDEATHSIG, SIGTERM);
		is_launcher = 1;

		//OT_LOG(LOG_ERR, "manager pid = %ld, launcher pid = %ld\n", (long)manager_pid, (long)control_params.launcher_pid);
	} else {
		/* child process will become the manager */
		close(sockfd[1]);
		...
		/* kill the manager after the launcher dies (e.g. on SIGTERM, SIGSEGV, SIGBUS, SIGILL or SIGFPE) */
		prctl(PR_SET_PDEATHSIG, SIGKILL);
		if (getppid() != control_params.launcher_pid)
			exit(1);
	}
	...
	/* start a test CA if required */
	if (test_ca) {
		/* start test CA from launcher */
		if (is_launcher) {
			/* wait for the manager to get ready */
			char d;
			if (read(sockfd[1], &d, 1) != 1 || d != 'r')
				exit(1);

			/* set signal to kill the launcher when the test CA (or the manager) stops */
			sigemptyset(&sig_act.sa_mask);
			sig_act.sa_flags = 0;
			sig_act.sa_handler = _exit;
			if (sigaction(SIGCHLD, &sig_act, NULL) == -1)
				exit(1);
			...
			case -1:
				...
				exit(1);
			case 0:
				/* child process starts the test CA */
				if (execl(test_ca, test_ca, NULL) == -1) {
					fprintf(stderr, "Failed to start \"%s\"\n", test_ca);
					OT_LOG(LOG_ERR, "Failed to start \"%s\"\n", test_ca);
				}
				exit(1); /* wrong app path? */
			default:
				/* parent process */
				break;
		        }
			...
			/* tell the launcher that the manager is ready */
			if (write(sockfd[0], "r", 1) != 1)
				exit(1);
		}
	}
